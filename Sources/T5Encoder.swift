...
public class MultiHeadAttention: Module {
    ...
    init(_ config: T5Configuration) {
        ...
        self._relativeAttentionBias.wrappedValue = Embedding(embeddingCount: config.relativeAttentionNumBuckets, dimensions: config.numHeads)
    }
    ...
    func callAsFunction(_ hiddenStates: MLXArray) -> MLXArray {
        ...
        let positionBias = computeBias(seqLength: seqLength)
    ...
    func computeBias(seqLength: Int) -> MLXArray {
        ...
        let relativePositionBucket = MultiHeadAttention.relativePositionBucket(relativePosition: relativePosition)
    ...
    static func relativePositionBucket(relativePosition: MLXArray, bidirectional: Bool = true, numBuckets: Int = 32, maxDistance: Int = 128) -> MLXArray {
        var relativeBuckets = MLXArray.zeros(relativePosition.shape, dtype: .int32)
        var numBucketsVar = numBuckets
        
        numBucketsVar /= 2
        relativeBuckets = relativeBuckets + MLX.where(relativePosition .> 0, MLXArray(numBucketsVar), MLXArray(0))
        ...
        let relativePositionIfLarge = maxExact + (MLX.log(relativePositionAbs.asType(.float32) / Float(maxExact)) * scale).asType(.int32)
        let relativePositionIfLargeClamped = MLX.minimum(relativePositionIfLarge, MLXArray(numBucketsVar - 1))
        
        relativeBuckets = relativeBuckets + MLX.where(isSmall, relativePositionAbs, relativePositionIfLargeClamped)
        return relativeBuckets
    }
}

public class T5Attention: Module,UnaryLayer {
...
public class T5FeedForward : Module,UnaryLayer {
...
public class T5Encoder: Module {
    ...
    public func callAsFunction(_ inputIds: MLXArray) -> MLXArray {
        ...
        let encoderOutputs = encoder(hiddenStates)
